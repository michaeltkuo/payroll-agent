  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <h2 className="text-lg font-semibold text-gray-900 mb-2">Reject Timecard</h2>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-gray-900">{tc.employee.name}</p>
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-600">{total.toFixed(2)} hrs</span>
            <span
              className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[tc.status] ?? "bg-gray-100 text-gray-700"}`}
            >
                ...
                {tc.entries
                  ...
                  .map((e) => (
                    <tr key={e.id} className="border-b border-gray-50 last:border-0">
                      ...
                      <td className="py-1.5 text-gray-600">{e.clock_in?.slice(0, 5) ?? "—"}</td>
                      <td className="py-1.5 text-gray-600">{e.clock_out?.slice(0, 5) ?? "—"}</td>
                      <td className="py-1.5 text-gray-700">
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Admin — Timecards</h1>
        <button
          onClick={load}
      ...
      {loading ? (
        ...
        <p className="text-gray-500">No timecards to review.</p>
      ) : (
